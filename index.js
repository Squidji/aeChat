...
io.sockets.on('connection', function(socket) {
    socket.on('username', function(username) {
		...
		io.emit('is_online', '🔵 <i><b style="color: '+c+';">' + socket.username + '</b> joined the chat..</i>');
		io.emit('upd_users', userlist, colorlist);
    });
    ...
    socket.on('disconnect', function(username) {
		if (socket.username !== undefined) {
			let c = userlist.indexOf(socket.username);
			if (c > colorlist.length-1) {c = '#3d91d6'} else {c = colorlist[c]};
			io.emit('is_online', '🔴 <i><b style="color: '+c+';">' + socket.username + '</b> left the chat..</i>');
